// import 'dart:convert';
// import 'package:encrypt/encrypt.dart';

// import 'package:pointycastle/export.dart';

// class ApisServices {
//   static const vodacomApi = "qIOdLnEHCqKdpBUCWeEGFnW2Xz0m9U7C";
//   static const vodacomSandboxPublicKey =
//       "";

//   static String vodacomPassKey() {
//     // Décoder la clé publique de la chaîne Base64
//     final publicKey =
//         RSAKeyParser().parse(vodacomSandboxPublicKey) as RSAPublicKey;

//     // Créer l'instance de chiffrement RSA
//     final encrypter = Encrypter(RSA(publicKey: publicKey));

//     // Chiffrer la clé API
//     final encrypted = encrypter.encrypt(vodacomApi);

//     // Encoder le résultat chiffré en Base64
//     final encoded = base64.encode(encrypted.bytes);

//     return encoded;
//   }
// }
